...
class User extends EloquentUser implements AuthenticatableContract,JWTSubject
{
    ...
    {
        return Cache::tags('users')->rememberForever(md5('users.list:' . locale()), function () {
            return self::join('user_roles', 'user_roles.user_id', '=', 'users.id')->where("user_roles.role_id",setting('customer_role'))->get()->sortBy('full_name')->pluck('full_name', 'id');
        });
    }
    ...
    {
        return Cache::tags('parents_list')->rememberForever(md5('users.list:' . locale()), function () {
            return self::all()->sortBy('full_name')->pluck('full_name', 'id');
        });
    }
